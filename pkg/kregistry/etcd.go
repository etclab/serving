...
type KeyRegistry struct {
	...
	Bgls03PublicKeys   map[string]*bgls03.PublicKey
	muBgls03PublicKeys sync.RWMutex
...
func (kr *KeyRegistry) SetupSignature(setupSignature bool, podId string) {
	...
	if setupSignature {
		...
		if err != nil {
			...
			logDev("Successfully obtained attestation report from ego")
		}
		...
		// list and watch for key value pairs starting with `bgls03/`
		// on receiving pp, pk, report for each function in the chain save it
		// verify the attestation report generated by pod
		go kr.ListWatchSignatureVerificationInfo()
	} else {
		logDev("AttachSignature is disabled, skipping bgls03 pp and key pair generation")
	}
}

// watches for bglg03 pp, pk, and attestation report required for verifying signatures
func (kr *KeyRegistry) ListWatchSignatureVerificationInfo() {
	logDev := mutil.LogWithPrefix("dev - ListWatchSignatureVerificationInfo")

	bgls03Prefix := "bgls03/"

	// only get public keys are public params are the same for all
	currentRevision := kr.FetchBgls03PublicKeys(bgls03Prefix)
	if currentRevision < 0 {
		logDev("Failed to fetch existing signature verification info, cannot start watch")
		return
	}

	rch := kr.Client().Watch(context.Background(), bgls03Prefix, clientv3.WithPrefix(), clientv3.WithRev(currentRevision+1))
	logDev("Watching etcd key: %s, rev: %d", bgls03Prefix, currentRevision+1)
	for wresp := range rch {
		if wresp.Canceled {
		...
		for _, ev := range wresp.Events {
			...
			if ev.Type == clientv3.EventTypePut {
				...
				err := kr.HandleBgls03PublicKeys(keyStr, value)
				if err != nil {
					logDev("Failed to save bgls03 keys: %v", err)
					continue
				}
			}
		}
...
func (kr *KeyRegistry) FetchBgls03PublicKeys(prefix string) int64 {
	logDev := mutil.LogWithPrefix("dev - FetchBgls03PublicKeys")

	reEncKeyRes, err := kr.Client().Get(context.Background(), prefix, clientv3.WithPrefix())
	if err != nil {
		logDev("failed to fetch existing bgls03 keys from etcd: %v", err)
		return -1
	}

	logDev("fetched %d existing bgls03 keys from etcd", len(reEncKeyRes.Kvs))
...
func (kr *KeyRegistry) HandleBgls03PublicKeys(keyStr string, value []byte) error {
	logDev := mutil.LogWithPrefix("dev - HandleBgls03PublicKeys")
	...
	if strings.HasSuffix(keyStr, "/pk") {
		pks := new(bgls.PublicKeySerialized)
		err = json.NewDecoder(bytes.NewReader(value)).Decode(pks)
		if err != nil {
			return fmt.Errorf("failed to decode bgls public key: %v", err)
		}

		pk, err = pks.DeSerialize()
		if err != nil {
			return fmt.Errorf("failed to deserialize bgls public key: %v", err)
		}

		kr.SafeWriteBgls03PublicKey(podId, pk)

	}

	if strings.HasSuffix(keyStr, "/pp") {
		pps := new(bgls.PublicParamsSerialized)
		err := json.NewDecoder(bytes.NewReader(value)).Decode(pps)
		if err != nil {
			return fmt.Errorf("failed to decode bgls public params: %v", err)
		}

		pp, err = pps.DeSerialize()
		if err != nil {
			return fmt.Errorf("failed to deserialize bgls public params: %v", err)
		}

		kr.SafeWriteBgls03PublicParams(podId, pp)

	}

	if strings.HasSuffix(keyStr, "/report") {
		err := json.NewDecoder(bytes.NewReader(value)).Decode(&attestationReport)
		if err != nil {
			return fmt.Errorf("failed to decode attestation report: %v", err)
		}

		report, err := enclave.VerifyRemoteReport(attestationReport)
		if err != nil {
			if err == attestation.ErrTCBLevelInvalid {
				// this is fine, we just have old microcode/firmware
				logDev("warning: TCB level invalid in report for podId %s", podId)
			} else {
				// this is bad
	...
	logDev("Stored bgls03 public key and params for podId %s", podId)
	return nil
}
...
func (kr *KeyRegistry) SafeWriteBgls03PublicParams(podId string, publicParams *bgls03.PublicParams) *bgls03.PublicParams {
	return mutil.GSafeWriteToMap(podId, publicParams, &kr.Bgls03PublicParams, &kr.muBgls03PublicParams)
}

func (kr *KeyRegistry) SafeWriteBgls03PublicKey(podId string, publicKey *bgls03.PublicKey) *bgls03.PublicKey {
	return mutil.GSafeWriteToMap(podId, publicKey, &kr.Bgls03PublicKeys, &kr.muBgls03PublicKeys)
}

func (kr *KeyRegistry) SafeReadBgls03PublicKey(podId string) *bgls03.PublicKey {
	return mutil.GSafeReadFromMap(podId, kr.Bgls03PublicKeys, &kr.muBgls03PublicKeys)
}
...
func (kr *KeyRegistry) VerifySignature(signature []byte, functionChain string, nonce string) error {
	...
	if functionChain == "" || nonce == "" || signature == nil {
		...
		for _, podId := range chain {
			...
			pk := kr.SafeReadBgls03PublicKey(podId)

			if pk == nil {
				logDev("public key not found for podId %s", podId)
				return fmt.Errorf("public key not found for podId %s", podId)
			} else {
				logDev("public key found verifying signature using public key of podId %s", podId)
			}
