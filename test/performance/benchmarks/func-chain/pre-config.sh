#!/bin/bash

set -e

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

# Default values
STRATEGY="${1:-knative}"
ns=default

echo "Configuring pre-config for strategy: $STRATEGY"

# PRE cryptographic keys (shared across strategies that use encryption)
# Client is the vegeta load generator (key pair, public key, public params)
CLIENT_KP='{"pk":{"g1_to_a":"AG/wwus8WJWrxzjJhhpRncAAhAdFIlpGu5yvuvKngNYEo9yn7j/GuaZkuCXuhksyBYjctO1o6Ez2x6sl8IcXcQqk1+ifh2yhDxv3Np1pW7HhLnawvU4ATaLEaAtJN9nJ","g2_to_a":""},"sk":"GAMl6pg83q06VkwUfLWR0k0O/xgTqP72fwSEJ0TE5x0="}'
CLIENT_PK='{"g1_to_a":"AG/wwus8WJWrxzjJhhpRncAAhAdFIlpGu5yvuvKngNYEo9yn7j/GuaZkuCXuhksyBYjctO1o6Ez2x6sl8IcXcQqk1+ifh2yhDxv3Np1pW7HhLnawvU4ATaLEaAtJN9nJ","g2_to_a":""}'
CLIENT_PP='{"g1":"F/HTpzGX15QmlWOMT6msD8NojE+XdLkFoU46PxcbrFhsVeg/+Xoa7/s68ArbIsa7CLP0geOqoPGgnjDtdB2K5Pz14JXV0Ar2ANsYyywEs+3QPMdEooiK5AyqIylGxefh","g2":"","z":""}'

# Leader keys (for leader and both strategies)
LEADER_KP='{"pk":{"g1_to_a":"GI2MQLdca2zEshws+rHnXX30+Y3nbWeuHkxyHX4y/0wPBD8SpDv3bLmGZcY+DWqgAHi+USuucatE+kjr9WiXa7ltYYZH/BghL4ZzHGbePkht+jyt1STK/olI4YTSZFsB","g2_to_a":""},"sk":"bSt58suS+D4WmPSsJ2CqEIul/clZz7inBqaHpdz7qbk="}'
LEADER_PK='{"g1_to_a":"GI2MQLdca2zEshws+rHnXX30+Y3nbWeuHkxyHX4y/0wPBD8SpDv3bLmGZcY+DWqgAHi+USuucatE+kjr9WiXa7ltYYZH/BghL4ZzHGbePkht+jyt1STK/olI4YTSZFsB","g2_to_a":""}'
LEADER_PP='{"g1":"F/HTpzGX15QmlWOMT6msD8NojE+XdLkFoU46PxcbrFhsVeg/+Xoa7/s68ArbIsa7CLP0geOqoPGgnjDtdB2K5Pz14JXV0Ar2ANsYyywEs+3QPMdEooiK5AyqIylGxefh","g2":"","z":""}'

# Member keys (for member and both strategies)
MEMBER_KP='{"pk":{"g1_to_a":"FkEttU1JHgpey6tR3J+SB5LmRGaR3Fo8BPLhm8JIQ7cXdXMOV7rUExycW6WekVq3Aq64BzPFi0fSDyNUDGPJKS9+kkMLgcFr/lElwjUGkqXSstwDG5K55NMU/FZDeBKd","g2_to_a":""},"sk":"BHV+ZaRA/489V2tXPyVDK22DNn4p66JObNfLG7iyOaI="}'
MEMBER_PK='{"g1_to_a":"FkEttU1JHgpey6tR3J+SB5LmRGaR3Fo8BPLhm8JIQ7cXdXMOV7rUExycW6WekVq3Aq64BzPFi0fSDyNUDGPJKS9+kkMLgcFr/lElwjUGkqXSstwDG5K55NMU/FZDeBKd","g2_to_a":""}'
MEMBER_PP='{"g1":"F/HTpzGX15QmlWOMT6msD8NojE+XdLkFoU46PxcbrFhsVeg/+Xoa7/s68ArbIsa7CLP0geOqoPGgnjDtdB2K5Pz14JXV0Ar2ANsYyywEs+3QPMdEooiK5AyqIylGxefh","g2":"","z":""}'

# Signature public params (BGLS)
SIGNATURE_PP='{"g1":"l/HTpzGX15QmlWOMT6msD8NojE+XdLkFoU46PxcbrFhsVeg/+Xoa7/s68ArbIsa7","g2":"k+ArYFJxn2B9rNOgiCdPZVlr0NCZILYatdphu9x/UEkzTPESE5RdV+WsfQVdBCt+AkqisvCPCpEmCAUnLcUQUcbketT6QDsCtFELZHrj0XcLrAMmqAW779SAVsjBIb24"}'

# Function chain definitions per strategy (must match service names in services/*.yaml)
FUNCTION_CHAIN_KNATIVE="validate-fun-stock/vote-fun-stock/count-vote-fun-stock/display-fun-stock"
FUNCTION_CHAIN_EFUNCTION="validate-fun/vote-fun/count-vote-fun/display-fun"
FUNCTION_CHAIN_LEADER="validate-fun-leader/vote-fun-leader/count-vote-fun-leader/display-fun-leader"
FUNCTION_CHAIN_MEMBER="validate-fun-svc/vote-fun-svc/count-vote-fun-svc/display-fun-svc"

# RSA key for rsa-efunction strategy
RSA_SK='-----BEGIN PRIVATE KEY-----

71FNC/BakPDBF2fXOQA7Ywvs
-----END PRIVATE KEY-----'

# Strategy-specific settings
case "$STRATEGY" in
  knative)
    FUNCTION_CHAIN="$FUNCTION_CHAIN_KNATIVE"
    FUNCTION_MODE="CHAIN"
    ATTACH_SIGNATURE="false"
    VERIFY_SIGNATURE="false"
    DISABLE_LOGGING="true"
    ;;
  efunction)
    FUNCTION_CHAIN="$FUNCTION_CHAIN_EFUNCTION"
    FUNCTION_MODE="CHAIN"
    ATTACH_SIGNATURE="false"
    VERIFY_SIGNATURE="false"
    DISABLE_LOGGING="true"
    ;;
  rsa-efunction)
    FUNCTION_CHAIN="$FUNCTION_CHAIN_EFUNCTION"
    FUNCTION_MODE="CHAIN"
    ATTACH_SIGNATURE="false"
    VERIFY_SIGNATURE="false"
    DISABLE_LOGGING="true"
    ;;
  leader-efunction)
    FUNCTION_CHAIN="$FUNCTION_CHAIN_LEADER"
    FUNCTION_MODE="CHAIN"
    ATTACH_SIGNATURE="true"
    VERIFY_SIGNATURE="false"
    DISABLE_LOGGING="true"
    ;;
  member-efunction)
    FUNCTION_CHAIN="$FUNCTION_CHAIN_MEMBER"
    FUNCTION_MODE="CHAIN"
    ATTACH_SIGNATURE="true"
    VERIFY_SIGNATURE="false"
    DISABLE_LOGGING="true"
    ;;
  both)
    FUNCTION_CHAIN="$FUNCTION_CHAIN_EFUNCTION"
    FUNCTION_MODE="CHAIN"
    ATTACH_SIGNATURE="true"
    VERIFY_SIGNATURE="false"
    DISABLE_LOGGING="true"
    ;;
  # both-sig verifies the signature too
  both-sig)
    FUNCTION_CHAIN="$FUNCTION_CHAIN_EFUNCTION"
    FUNCTION_MODE="CHAIN"
    ATTACH_SIGNATURE="true"
    VERIFY_SIGNATURE="true"
    DISABLE_LOGGING="true"
    ;;
  # both-hash-chain-sig verifies signatures and uses hash chain for leader key verification
  both-hash-chain-sig)
    FUNCTION_CHAIN="$FUNCTION_CHAIN_EFUNCTION"
    FUNCTION_MODE="CHAIN"
    ATTACH_SIGNATURE="true"
    VERIFY_SIGNATURE="false"
    DISABLE_LOGGING="false"
    FLOW_TRACKING_ENABLED="true"
    ;;
  *)
    echo "Unknown strategy: $STRATEGY"
    echo "Available strategies: knative, efunction, rsa-efunction, leader-efunction, member-efunction, both, both-sig, both-hash-chain-sig"
    exit 1
    ;;
esac

# Delete existing secret
kubectl delete secret pre-config -n $ns --ignore-not-found=true

# Build secret creation arguments
SECRET_ARGS=(
  "--from-literal=strategy=${STRATEGY}"
  "--from-literal=leader_kp=${LEADER_KP}"
  "--from-literal=leader_pk=${LEADER_PK}"
  "--from-literal=leader_pp=${LEADER_PP}"
  "--from-literal=member_kp=${MEMBER_KP}"
  "--from-literal=member_pk=${MEMBER_PK}"
  "--from-literal=member_pp=${MEMBER_PP}"
  "--from-literal=client_kp=${CLIENT_KP}"
  "--from-literal=client_pk=${CLIENT_PK}"
  "--from-literal=client_pp=${CLIENT_PP}"
  "--from-literal=function_chain=${FUNCTION_CHAIN}"
  "--from-literal=function_mode=${FUNCTION_MODE}"
  "--from-literal=attach_signature=${ATTACH_SIGNATURE}"
  "--from-literal=verify_signature=${VERIFY_SIGNATURE}"
  "--from-literal=signature_pp=${SIGNATURE_PP}"
  "--from-literal=disable_logging=${DISABLE_LOGGING}"
  "--from-literal=flow_tracking_enabled=${FLOW_TRACKING_ENABLED:-false}"
)

# Only include RSA_SK for rsa-efunction strategy
if [[ "$STRATEGY" == "rsa-efunction" ]]; then
  SECRET_ARGS+=("--from-literal=rsa_sk=${RSA_SK}")
fi

# Create the secret
kubectl create secret generic pre-config -n $ns "${SECRET_ARGS[@]}"

echo "Pre-config secret created for strategy: $STRATEGY"
echo "  FUNCTION_MODE: $FUNCTION_MODE"
echo "  ATTACH_SIGNATURE: $ATTACH_SIGNATURE"
echo "  FUNCTION_CHAIN: $FUNCTION_CHAIN"
